function [Tcorr t cross_prod_binned]= compute_ACF(x,dt, bin_c, varargin) % varargin method options, empty default
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% bin_c:  window size for local(block) averages. The actual block-time =
% maximum sampling time* bin_c
% ----Optional input------------------------------------------------------
...
if ~isempty(varargin)
   ...
   for m = 1: size(x_sqz,1)
     ...
     for j = 1 : nt
        ...
        loc_m_del = double(binning_1d(x1(lag+1:end), prod_bsize* bin_c,'double'  ))/(prod_bsize* bin_c) ;
        loc_m_dir = double(binning_1d(x1(1:end-lag), prod_bsize* bin_c,'double' ))/(prod_bsize* bin_c) ;        
