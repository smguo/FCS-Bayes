...
% Jan 20, 2012
% -----------------------------------------------------------------
function [a std_beta logML residual logCOVB log_resid] = FCS_fit_bayes_im(time,corr,k,opt,model,noise,L,sigma,varargin)
%% Option for models
switch model
...
%     a0(4) = 1-a0(1)/mean(corr(1:5)); 
    ...
    a0 = zeros(1,3);
    a0(1) = (max(corr) - min(corr));
    a0(2) = (L+sigma)^2/4/time(find(ismember(abs((max(corr) - min(corr))/2 - corr),min(min(abs((max(corr) - min(corr))/2 - corr)))),1,'first'));
    a0(3) = max([min(corr) 0]);    
    f1 = @(a,t)diff1com2D_im(a,t,L,sigma) ;
    
    case '2comp2Ddiff_im'
    a0 = zeros(1,5);        
    a0(1) = (max(corr) - min(corr))/2;  a0(2) = a0(1);
    a0(3) = (L+sigma)^2/4/time(find(ismember(abs((max(corr) - min(corr))/2 - corr),min(min(abs((max(corr) - min(corr))/2 - corr)))),1,'first'));
    a0(4) = a0(3);
    a0(5) = max([min(corr) 0]);
    f1 = @(a,t)diff2com2D_im(a,t,L,sigma) ;
    
    case '3comp2Ddiff_im'
    a0 = zeros(1,7);        
    a0(1) = (max(corr) - min(corr))/2;  a0(2) = a0(1); a0(3) = a0(1);
    a0(4) = (L+sigma)^2/4/time(find(ismember(abs((max(corr) - min(corr))/2 - corr),min(min(abs((max(corr) - min(corr))/2 - corr)))),1,'first'));
    a0(5) = a0(4) ; a0(6) = a0(4) ;
    a0(7) = max([min(corr) 0]);
    f1 = @(a,t)diff3com2D_im(a,t,L,sigma) ;    
