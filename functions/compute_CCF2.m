...
% Apr 06, 2012
% -----------------------------------------------------------------
function [Tcorr t cross_prod_binned]= compute_CCF2(xa,xb,dt,block_min_ind, varargin)
...
%     cross_prod_binned = zeros(floor(n_prod_min/bin_c),size(t,2), size(xa,1));   
    cross_prod_binned = cell(size(xa,1));
     for m = 1: size(xa,1)       
        xa1 = xa(m,:) ;
        xb1 = xb(m,:) ;
         incre_current = 1;
...
%             cross_prod_binned(:,j,m) = cross_prod_block_s(1:floor(n_prod_min/bin_c))./mean_del./mean_dir ;    
            cross_prod_binned{m}(:,j) = cross_prod_block_s(1:floor(n_prod_min/bin_c))./mean_del./mean_dir ; 
